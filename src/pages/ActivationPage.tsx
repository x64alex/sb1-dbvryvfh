...
export const ActivationPage = () => {
  ...
  useEffect(() => {
    const checkSubscriptionHistory = async () => {
      try {
        ...
        const hadPreviousSubscription = subscription.subscription.category !== 'Basic' || 
          (subscription.subscription.category === 'Basic' && !subscription.subscription.is_active);

        setIsReactivation(hadPreviousSubscription);
        if (hadPreviousSubscription) {
          setPreviousPlan(subscription.subscription.category.toLowerCase());
          setSelectedPlan(subscription.subscription.category.toLowerCase());
        }
      } catch (error) {
  ...
  const handleContinue = () => {
    ...
    toast.success(`Successfully ${isReactivation ? 'reactivated' : 'activated'} ${plan.name} Plan`);
    navigate('/settings/subscription');
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-16">
        ...
        <div className="flex items-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            {isReactivation ? 'Reactivate your subscription' : 'Select your subscription'}
          </h1>
        </div>
        ...
        <div className="mt-12 text-center">
          ...
          >
            {isReactivation ? 'Reactivate' : 'Start Free Trial with'} {plans.find(p => p.id === selectedPlan)?.name}
          </button>
          {!isReactivation && (
